...
if (import.meta.env.DEV) {
  console.log(
    "🔧 GitHub API debugging enabled. Check console for detailed feedback submission logs."
  );
}
...
if ("requestIdleCallback" in window) {
  requestIdleCallback(
    ...
    { timeout: 100 }
  ); // Short timeout to ensure it runs quickly but not during critical rendering
} else {
          ...
          <BrowserRouter>
            <Suspense fallback={<LoadingFallback />}>
              <Routes>
                <Route path="/" element={<App />} />
                <Route
                  path="/algorithms/config/:algorithmName"
                  element={<App />}
                />
                <Route
                  path="/algorithms/details/:algorithmName"
                  element={<App />}
                />
                <Route
                  path="/algorithms/metrics/:algorithmName"
                  element={<App />}
                />
